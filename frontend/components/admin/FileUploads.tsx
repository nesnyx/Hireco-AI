...
const FileUploads = () => {
    ...
    const handleJobSelect = (jobId : any) => {
    ...
    const handleFiles = (newFiles : any) => setFiles((prev:any) => [...prev, ...Array.from(newFiles)]);
    ...
    const handleDrop = useCallback((e : any) => { e.preventDefault(); setIsDragging(false); handleFiles(e.dataTransfer.files); }, []);
    const handleFileSelect = (e : any) => handleFiles(e.target.files);
    const removeFile = (idx : any) => setFiles((prev:any) => prev.filter((_ : any, i:any) => i !== idx));
                        ...
                        <div>
                            ...
                            {loading ? <p>Memuat profil...</p> : jobError ? <p className="text-red-400">{jobError}</p> : (
                                <select id="job-select" value={selectedJobId} onChange={(e) => handleJobSelect(e.target.value)} className="w-full md:w-2/3 bg-slate-900 border border-slate-600 rounded-lg p-3 focus:ring-2 focus:ring-indigo-500">
                                    ...
                                    {data.length > 0 ? (data.map((job:any) => <option key={job.id} value={job.id}>{job.title}</option>)) : (
                            ...
                            <div className="space-y-3 max-h-72 overflow-y-auto pr-2">
                                {files.map((file:any, index:any) => (
                                    <div key={index} className="flex items-center justify-between bg-slate-800 p-3 rounded-lg">
                                        <div className="flex items-center gap-4 overflow-hidden"><FiFileText className="text-2xl text-slate-400 shrink-0" />
