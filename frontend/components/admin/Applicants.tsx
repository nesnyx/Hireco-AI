...
export default function Applicant() {
    ...
    const stats = useMemo(() => {
        ...
        const highScores = data?.filter((app :any) => app.score >= 80).length || 0;
        const avg = total > 0 ? Math.round(data.reduce((acc : any, app : any) => acc + (app.score || 0), 0) / total) : 0;
...
function Badge({ score, config } :any) {
    return (
